...
<!--Introduction-->
<!--Jakub Nowosad and Tomasz Stepinski (2018). sabre: Spatial Association Between REgionalizations. R package version 0.1.0.-->
...
- the V-measure method <!--ref to our paper-->
...
It can be done with `sabre_calc()`, which calculates "V-measure", "Homogeneity", and "Completeness" and returns two preprocessed input maps. 
...
The `sabre_calc()` function internally calculates intersections of the input geometries using `sf::st_intersection()`, which depends on the coordinates values precision. 
The `precision` argument can be used when `sabre_calc` produces an error.
